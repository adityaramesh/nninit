...
for _, scale in pairs({1, 2, 3, 10, 11}) do
	local m = nninit.make_spatial_downsampling_conv({kernel_width = scale,
...
for _, scale in pairs({1, 2, 3, 10, 11}) do
	local m = nninit.make_spatial_downsampling_conv({kernel_width = 2 * scale,
		n_maps_in = 3, expansion_factor = 1, scale = scale, input_width = img_width})
...
for _, scale in pairs({1, 2, 3, 10, 11}) do
	local m = nninit.make_spatial_downsampling_conv({kernel_width = 2 * scale + 1,
		n_maps_in = 3, expansion_factor = 1, scale = scale, input_width = img_width})
