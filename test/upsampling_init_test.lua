...
for _, scale in pairs({1, 2, 3, 10, 11}) do
	local m = nninit.make_spatial_upsampling_conv({kernel_width = 2 * scale - 1,
		n_maps_in = 3, expansion_factor = 1, scale = scale})
...
for _, scale in pairs({1, 2, 3, 10, 11}) do
	local m = nninit.make_spatial_upsampling_conv({kernel_width = 4 * scale,
		n_maps_in = 3, expansion_factor = 1, scale = scale})
